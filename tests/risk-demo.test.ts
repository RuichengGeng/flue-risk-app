...
test('runs generated Python in the coding sandbox and writes an Excel artifact', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'flue-var-demo-'));
  const input = join(dir, 'coding-sandbox.json');
  await writeFile(
    input,
    JSON.stringify({
      task: 'Exclude WTI, aggregate component VaR by asset class, and export the result.',
      tables: {
        component_risk: [
          { asset_class: 'Commodity', product: 'Brent', component_var: 760_000 },
          { asset_class: 'Commodity', product: 'WTI', component_var: 280_000 },
          { asset_class: 'Equity', product: 'AAPL', component_var: 160_000 },
        ],
      },
      python_code: `
rows = [row for row in tables["component_risk"] if row["product"] != "WTI"]
totals = {}
for row in rows:
    asset_class = row["asset_class"]
    totals[asset_class] = totals.get(asset_class, 0) + row["component_var"]
summary_rows = [{"asset_class": key, "component_var": value} for key, value in sorted(totals.items())]
write_excel("coding-sandbox-test", summary_rows, "Summary")
result = {
    "summary": "Excluded WTI and aggregated component VaR by asset class.",
...
`,
  ...
  const { stdout } = await execFileAsync('python3', ['python/coding_sandbox.py', '--input', input]);
  const result = JSON.parse(stdout) as {
    status: string;
    tables: { summary: Array<{ asset_class: string; component_var: number }> };
    artifacts: Array<{ kind: string; path: string }>;
  };

  assert.equal(result.status, 'ok');
  assert.deepEqual(result.tables.summary, [
    { asset_class: 'Commodity', component_var: 760_000 },
    { asset_class: 'Equity', component_var: 160_000 },
  ]);
  assert.equal(result.artifacts[0].kind, 'excel');
  const bytes = await readFile(result.artifacts[0].path);
  assert.equal(bytes.subarray(0, 2).toString('utf8'), 'PK');
});
